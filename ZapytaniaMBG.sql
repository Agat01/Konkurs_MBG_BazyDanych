...
where r.nazwa='region dolnoœl¹sko-opolski';
...
from powiat p join wojewodztwo w on(p.id_wojewodztwo=w.id_wojewodztwo)
              ...
              join klasa k on(k.id_szkola=sz.id_szkola)
where (w.nazwa='dolnoœl¹skie' or w.nazwa='opolskie') and (k.id_typ_klasy=1 or k.id_typ_klasy=2);
...
from powiat p join wojewodztwo w on(p.id_wojewodztwo=w.id_wojewodztwo)
              join szkola sz on(sz.id_powiat=p.id_powiat)
              join klasa k on(k.id_szkola=sz.id_szkola)
where (w.nazwa='dolnoœl¹skie' or w.nazwa='opolskie');
